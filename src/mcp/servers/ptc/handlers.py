...
async def run_python_impl(
    ...
    *,
    ...
    # and the tmpdir is not visible, so the LOCAL bridge cannot work. The
    # file-based-RPC-over-``docker exec`` variant is not implemented in this
    # build — refuse rather than silently run on the host.
    # TODO(ptc-docker): implement file-based RPC over ``docker exec`` (base64
    # ship the script+prelude into the container, poll request/response files),
    # mirroring the reference ``code_execution_tool._execute_remote``.
    if backend.name == "docker":
        return _refuse(
            "PTC does not yet support the docker sandbox backend "
            "(file-based RPC is unimplemented); refusing to run on the host.",
            t0=t0,
        )
