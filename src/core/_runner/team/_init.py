...
def background_executor(team: "Team") -> Any:
    ...
    if team._background_executor is None:
        ...
        team._background_executor = ThreadPoolExecutor(max_workers=3, thread_name_prefix="openagent-bg")
...
def _set_debug(team: "Team", debug_mode: Optional[bool] = None) -> None:
    ...
    debug_level: Literal[1, 2] = (
        cast(Literal[1, 2], int(env)) if (env := getenv("OPENAGENT_DEBUG_LEVEL")) in ("1", "2") else team.debug_level
    )
    # If the default debug mode is set, or passed on run, or via environment variable, set the debug mode to True
    if team.debug_mode or debug_mode or getenv("OPENAGENT_DEBUG", "false").lower() == "true":
...
def _set_memory_manager(team: "Team") -> None:
    ...
    team.add_memories_to_context = False
...
def initialize_team(team: "Team", debug_mode: Optional[bool] = None) -> None:
    ...
    # Memory + culture + compression are delegated subsystems (vision
    # §5 — vault MCP) or out of scope. The initializers are no-ops; the
    # call sites are left so an external caller passing those flags
    # gets a deterministic "the flag is honoured as off" result.
