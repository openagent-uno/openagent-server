...
class OpenAgentLogger(logging.Logger):
...
def build_logger(logger_name: str, source_type: Optional[str] = None) -> Any:
    # If a logger with the name "openagent.{source_type}" is already
    # set, reuse it.
    _logger = logging.getLogger(f"{LOGGER_NAME}.{logger_name}")
    if _logger.handlers or _logger.level != logging.NOTSET:
        return _logger

    # Set the custom logger class as the default for this logger
    logging.setLoggerClass(OpenAgentLogger)
    ...
    rich_handler = ColoredRichHandler(
        ...
        show_path=True if getenv("OPENAGENT_RUNTIME") == "dev" else False,
...
agent_logger: OpenAgentLogger = build_logger(LOGGER_NAME, source_type="agent")
team_logger: OpenAgentLogger = build_logger(TEAM_LOGGER_NAME, source_type="team")
workflow_logger: OpenAgentLogger = build_logger(WORKFLOW_LOGGER_NAME, source_type="workflow")

# Set the default logger to the agent logger
logger: OpenAgentLogger = agent_logger
...
# Opt-in only: set OPENAGENT_LOG_TRACEBACKS=true or call set_log_tracebacks(True).
log_tracebacks: bool = getenv("OPENAGENT_LOG_TRACEBACKS", "false").lower() in ("true", "1", "yes")
...
def _using_default_logger(logger_instance: Any) -> bool:
    """Return True if the currently active logger is our default OpenAgentLogger"""
    return isinstance(logger_instance, OpenAgentLogger)
...
def log_warning(msg, *args, **kwargs):
    """Log a warning. When called inside an except block and OPENAGENT_LOG_TRACEBACKS
...
def log_error(msg, *args, **kwargs):
    """Log an error. When called inside an except block and OPENAGENT_LOG_TRACEBACKS
...
def log_exception(msg, *args, **kwargs):
    ...
    regardless of OPENAGENT_LOG_TRACEBACKS setting.
    """
    global logger
...
def set_log_tracebacks(enabled: bool = True) -> None:
    ...
    Can also be controlled via the ``OPENAGENT_LOG_TRACEBACKS`` env var.
    """
...
def configure_agno_logging(
    ...
    """
    Util to set custom loggers. These will be used everywhere across the runtime.
    ...
    Args:
        ...
        enable_log_tracebacks: If True, show full tracebacks on warning logs.
            Also controllable via ``OPENAGENT_LOG_TRACEBACKS`` env var.
    """
