...
def inject_openagent_client_header(client_params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Add the `x-goog-api-client: openagent/<version>` header to the Gemini http_options.

    Google requests partner integrations to identify themselves via this header
    (https://ai.google.dev/gemini-api/docs/partner-integration#client-id). Using
    `x-goog-api-client` rather than User-Agent leaves any user-supplied UA intact.
    """
    try:
        from importlib.metadata import version as _pkg_version

        client_version = _pkg_version("openagent-framework")
    except Exception:
    ...
    if isinstance(http_options, dict):
        ...
        if isinstance(headers, dict):
            headers["x-goog-api-client"] = f"openagent/{client_version}"
