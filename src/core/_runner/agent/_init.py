...
def set_debug(agent: Agent, debug_mode: Optional[bool] = None) -> None:
    ...
    debug_level: Literal[1, 2] = (
        cast(Literal[1, 2], int(env)) if (env := getenv("OPENAGENT_DEBUG_LEVEL")) in ("1", "2") else agent.debug_level
    )
    # If the default debug mode is set, or passed on run, or via environment variable, set the debug mode to True
    if agent.debug_mode or debug_mode or getenv("OPENAGENT_DEBUG", "false").lower() == "true":
...
def set_telemetry(agent: Agent) -> None:
    ...
    telemetry_env = getenv("OPENAGENT_TELEMETRY")
...
def set_default_model(agent: Agent) -> None:
    ...
    if agent.model is None:
        try:
            ...
            log_exception(e)
            raise ImportError(
                "Runtime agents use `openai` as the default model provider. Please provide a `model` or install `openai`."
            ) from e
...
def set_culture_manager(agent: Agent) -> None:
    """No-op. Culture/i18n is out of scope for OpenAgent."""
    agent.add_culture_to_context = False


def set_memory_manager(agent: Agent) -> None:
    """No-op. User-memory curation is delegated to the vault MCP (vision §5).

    Kept as a named symbol so existing call sites in ``_messages.py``
    (lazy init at ``add_memories_to_context`` read time) can still
    import and call it without raising. The agent's ``memory_manager``
    stays ``None`` and ``add_memories_to_context`` stays falsy, so the
    downstream memory-fetch branches are skipped.
    """
...
def initialize_agent(agent: Agent, debug_mode: Optional[bool] = None) -> None:
    ...
    set_id(agent)
    set_telemetry(agent)
    # User-memory curation is delegated to the vault MCP (vision §5).
    # The fields below survive as inert flags for backwards-compat; their
    # initializers are intentionally not invoked. Same for culture (i18n)
    # and per-tool-result compression — both are out-of-scope subsystems
    # for OpenAgent and the stub-only managers would no-op anyway.
