...
class Agent:
    ...
    def background_executor(self) -> Any:
        if self._background_executor is None:
            ...
            self._background_executor = ThreadPoolExecutor(max_workers=3, thread_name_prefix="openagent-bg")
    ...
    def get_user_memories(self, user_id: Optional[str] = None) -> Optional[List[UserMemory]]:
        # User-memory curation lives in the vault MCP (vision §5). This
        # method is preserved for API back-compat but always returns None;
        # the runtime no longer maintains a per-user memory table.
        return None
