...
async def _h_if(
    ...
    # ``if not expr`` would treat a legitimately-False Jinja result as
    # missing config and raise. Only the *absent-key* case is an
    # invalid graph; a False value is a normal branch input that must
    # flow to the "false" handle below.
    if expr is None:
