...
class AgnoProvider(BaseModel):
    ...
    def __init__(
        ...
        self._tool_cache: list[Callable[..., Awaitable[str]]] | None = None
    ...
    def _build_tool_function(self, tool_def: dict[str, Any]) -> Callable[..., Awaitable[str]]:
        ...
        exec("\n".join(source_lines), {"Any": Any, "_tool_handler": _tool_handler}, namespace)
        func = namespace[func_name]
        func.__name__ = func_name
        func.__doc__ = tool_def.get("description") or f"MCP tool: {tool_name}"
        return func

    def _build_tools(self) -> list[Callable[..., Awaitable[str]]]:
        if self._tool_cache is not None:
            return self._tool_cache
        if not self._mcp_registry:
            self._tool_cache = []
            return self._tool_cache
        self._tool_cache = [self._build_tool_function(tool_def) for tool_def in self._mcp_registry.all_tools()]
        return self._tool_cache
    ...
    def _ensure_agent(self):
        ...
        try:
            ...
            raise RuntimeError("Agno is not installed. Install the 'agno' dependency to use API-backed models.") from exc
